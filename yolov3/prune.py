...
##  output a new cfg file
    ...
    for i, (module_def, module) in enumerate(zip(model.module_defs, model.module_list)):
        ...
        if mtype  == 'convolutional':
            ...
            if bn:
                ...
                mask = weight_copy.gt(thresh).float().cuda()  # 掩模
                
                if int(torch.sum(mask)) < min_channel_num: 
                    _, sorted_index_weights = torch.sort(weight_copy,descending=True)
                    mask[sorted_index_weights[:min_channel_num]]=1. # 保持维度
    ...
    for i in range(layer_number-1, -1, -1):
        ...
        if mtype == 'shortcut': # 是shortcut
            if proned_module_defs[i]['is_access']: # 已经访问过
                continue
            ...
            while mtype == 'shortcut':
                ...
                if proned_module_defs[layer_i-1]['type'] == 'convolutional': # 上一层为conv，判断是否有bn
                    bn = int(proned_module_defs[layer_i-1]['batch_normalize'])
                    if bn: # 有BN，则有mask：
                        Merge_masks.append(proned_module_defs[layer_i-1]["mask"].unsqueeze(0))

                layer_i = int(proned_module_defs[layer_i]['from'])+layer_i # shortcut那一支
            ...
            while mtype == 'shortcut':
                ...
                if mtype == 'convolutional': # 上一层为conv，判断是否有bn
                    bn = int(proned_module_defs[layer_i]['batch_normalize'])
                    if bn:     
        ...
        if mtype  == 'convolutional':
            ...
            if bn:
                ...
                if i > 0:
                    ...
                    proned_module_defs[i]['mask_before'] = torch.tensor(mask_before).float().cuda()  # 掩模
